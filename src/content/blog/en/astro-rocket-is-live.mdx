...
**Hero radial gradient** — the homepage hero has a radial brand-colour glow that bleeds in from above the viewport. Light mode gets a subtle 12% opacity wash; dark mode gets a richer 38%. The effect uses CSS relative colour (`oklch(from var(--brand-500) l c h / 0.38)`) to pull the hue directly from the active brand token, so it adapts automatically to whichever of the 12 colour themes is active. No JavaScript, no hardcoded colours. The homepage header is a floating capsule that lets the glow show through behind it. On all other pages the header is a full-width solid bar. [Read the full post](/blog/dark-mode-hero-gradient).
